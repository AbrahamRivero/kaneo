...
import { ArrowUpRight, ChartLine, Ellipsis } from "lucide-react";
...
function CustomTooltip({
  ...
  if (active && payload && payload.length) {
    return (
      <div className="rounded-lg border border-border p-2 shadow-lg">
...
export function BurndownChart() {
  ...
  return (
    <div className="rounded-xl border border-border p-4 sm:p-6 flex-[1.5] min-w-0 bg-muted/70 dark:bg-muted/50">
      <div className="flex items-center justify-between mb-4 sm:mb-6">
        <div className="flex items-center gap-2.5">
          <div className="flex items-center justify-center size-8 rounded-lg border border-border shrink-0">
            <ChartLine className="size-[18px] text-muted-foreground" />
          </div>
          <span className="text-sm font-medium leading-tight">Burndown Chart</span>
        </div>
        <DropdownMenu>
